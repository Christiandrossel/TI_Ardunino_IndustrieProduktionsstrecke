...
\todo[author=anja]{Anpassungen nach Rückmeldung}

\section{Vorbereitung}
%TODO einiges evtl. einkürzen da es sich mit der Umsetzung schneidet.
\todo[author=anja, inline]{Ich hätte jetzt unter Vorbereitung erstmal nur das Annähern an und ausprobieren der Technik mit der gearbeitet werden soll (arduino starterkitprojekte + modifikationen + notepad++) um davon ausgehend auch ideen fürs konzept und die umsetzung entwickeln zu können, verstanden. daher hätte ich jetzt hier einen eher kurzen einleitungsteil erwartet und erst nach den ganzen modifiaktionsbeispielen und dem notepad++ abschnitt eine section in ders um die entwicklung des konzepts und die festgelegten anforderungen geht.}
Um ein Projekt in dieser Größe zu erstellen, wurde ein Konzept erarbeitet. Diese Vorüberlegungen dienten dazu, Schwerpunkte zu erkennen und zu analysieren. Als Basis standen mehrere „Turtle-Robot“-Bausätze zur Verfügung, welches ein Arduino als Basis beinhaltet. Außerdem wurden „Arduino-Starter-Kit“ verwendet, die eine Reihe an Bauteilen mitbringen. Die Roboter waren unter anderem mit Tracking Sensoren ausgestattet und so konstruiert, dass sie selbständig auf einen vorgegebenen Weg entlangfahren konnten. Außerdem besitzen sie Ultraschallsensoren, um auf einer bestimmten Distanz vor sich ein Objekt wahrzunehmen. \todo[author=anja,inline]{Plötzlicher Wechsel von Vergangenheit zu Gegenwart. würde eher gegenwart nehmen}  
...
Des Weiteren ergibt sich eine weitere Herausforderung, das selbständige stehenbleiben und losfahren an den Maschinen, der Roboter \todo[author=anja]{vermutlich meinst du: \ldots der Roboter an den Maschinen.}. Um sich diese Herausforderung und deren möglichen Lösungen heranzuführen,\todo[author=anja]{Das Verb passt nicht. kann sein, dass du zwei verben brauchst.} wurde ein Paper Prototyp entwickelt. Damit konnten Szenarien visuell dargestellt werden und weitere Schwerpunkte festgestellt werden. Für die Erkennung, dass ein Roboter an einer Station steht, wurden Lösungsmöglichkeiten wie Fotowiderstände, Schranken und Breakpoints gefunden. Durch Fotowiderstände an den Stationen könnte das Licht gemessen werden. Bei Veränderungen des Lichts kann daraus geschlussfolgert werden, dass sich ein Roboter an der Station befindet. Die Schranke soll als Hindernisobjekt dienen und mit einem Servomotor realisiert werden können, welches der Roboter wahrnehmen kann und stehen bleibt. Der Breakpoint ist eine Unterbrechung der Schwarzen Linie auf dem sich die Roboter bewegen. Der Roboter könnte dies erkennen und bleibt stehen. Die Stationen sollen zudem ein Signal für den Betrachter zurückgeben. Dadurch kann für den Betrachter erkenntnis- gewonnen werden ob Maschine Roboter erkennt.\todo[author=anja]{Nochmal durchlesen :-)} Die Überlegung ist hier dabei mit LED’s, Piezo oder eine Fortschrittsanzeige mit einem LCD Bildschirm.\todo[author=anja]{was zu tun? da fehlt ein verb}
...
Für die Einarbeitung des Projektes mit Arduino wurde "Das Arduino-Starter-Kit"\todo[author=anja]{muss nicht alles in anführungszeichen und wenn, dann bitte \textbackslash{}enquote\{\} verwenden} verwendet. Dieses beinhaltet neben dem Arduino und der Steckplatine, weitere Bauteile und ein Projektbuch. Nachdem die Einarbeitung durch das Buch gelungen ist, wurden einige Projekte so abgeändert, dass sie das Verständnis der Bauteile, des Schaltungsaufbau und der Programmierung näherbringen. Einige dieser, für das Projekt relevante abgeänderten Starterkit-Projekte sind in dem nachfolgenden Kapitel aufgeführt und näher beschrieben. Des Weiteren wird in Kapitel XY\todo[author=anja]{Konkrete Verweise auf andere Kapitel/Abschnitte sind oft eher unüblich vlt einfach mit anschließend sätze verbinden?} auf die Einstellung von Notepad ++ eingegangen.
\todo[author=anja,inline]{der letzte teil geht schon eher in die richtung, die ich mir unter der überschrift vorbereitung vorgestellt hatte}
...
Das Starterkit bringt eine geführte Einleitung in die Welt des Arduinos.\todo[author=anja]{hier irgendwo projektbuch erwähnen und quelle dazu} Durch den Aufbau von Schaltungen mit den entsprechenden Bauteilen und dem anschließenden Programmieren, wird das Verständnis übermittelt.
...
Für dieses Projekt wird die RGB-LED verwendet.\todo[author=anja]{Projekt bennenen, inklusive nummer und quelle} Der Nutzer\todo[author=anja]{weg mit dem nutzer!} möchte, dass diese LED in verschiedenen Farbtönen aufleuchtet. Außerdem soll der Benutzer entscheiden können, welche Farbe gerade aufleuchten soll. Genauso soll er das Licht auch einfach wieder ausschalten können. Die Auswahl des Farbtones \todo[author=anja]{Auswahl an Farbtönen?} soll sehr groß sein. Die LED soll einfach zu bedienen sein. Ss soll nicht für jeden Farbton ein einzelner Schalter angelegt werden.
\\
...
\end{itemize}
\todo[author=anja,inline]{weg damit. ist nicht vollständig und grundaufbau dem bild entnehmbar}
...
Als Eingabekomponente wird der Potentiometer empfohlen, da er sich durch drehen einfach bedienen lässt. Außerdem hat man\todo[author=anja]{Koalas auf dem Gewissen} mit diesem Schalter die Möglichkeit eine Vielzahl an unterschiedlichen Farben zu erzeugen und es\todo[author=anja]{was?} auch auszuschalten. 
...
\todo[author=anja,inline]{mir ist jetzt nicht klar, was das ausgangsprojekt war und worin deine modifikation besteht}
\todo[author=anja,inline]{du beschreibst ziemlich viel den quellcode. das ist eher uninteressant (basiscode kommt ja auch mit der ide). das grundlegende prinzip und ein oder zwei ausgewählte aspekte aus dem quellcode, die für die umsetzung irgendwie wichtig waren, reichen aus.}
\todo[author=anja,inline]{ein paar absätze (zwei mal enter drücken) wären ganz schön}
...
Damit der Arduino die Annäherung an den Sensor erkennen kann, benötigt der Lichtsensor einen Schwellwert, bei dessen Unterschreiten die Alarmanlage ausgelöst wird. Dies wird in der setup()-Funktion nach der Initialisierung der Sensoren in einem 5 Sekunden langen Zeitfenster in der der Benutzer einmal den Sensor abdecken muss.\todo[author=anja]{lies nochmal :-)} Es wird der maximal- und der minimal Wert\todo[author=anja]{Maximalwert oder maximal gemessene Wert, bei minimal dito} des Sensors genommen und der Schwellwert daraus berechnet. Während diesen 5 Sekunden blinkt die rote LED, um dem Nutzer zu signalisieren, dass sich der Arduino noch initialisiert.
...
\end{itemize}
\todo[author=anja,inline]{Würd ich normal in den Text schreiben. Haben sonst auch keine Anstriche für sowas}
...
\end{center}
\end{figure}
\todo[author=anja,inline]{was macht das bild in dem abschnitt?}
...
Es handelt sich um eine Widerstandsmatrix, mit deren Hilfe man\todo[author=anja]{den Meerespiegel erhöhen kann.} an einem analogen Eingang des Arduinos auslesen kann,
welcher Knopf vom Nutzer betätigt wurde. Dadurch ist es nicht nötig einen Eingang pro Knopf zu benutzen.
Um die Oktaven bequem\todo[author=anja]{bequem klingt bissl unpassend, vlt lieber ne variante mit einfach oder leicht} umschalten zu können, wird ein Potentiometer verwendet. Die möglichen Stellungen des Potentiometers (0-1023) werden in vier Bereiche aufgeteilt und dementsprechend ein Offset erhalten mit dem die Frequenzen der niedrigsten Oktave multipliziert werden.
...
Die Beiden nachfolgenden Abbildungen\todo[author=anja]{APA verbietet das. immer direkte referenz} zeigen den Sourcecode und bieten nähere Erklärungen.
...
Für das Projekt Windrad werden neben einem Gleichstrommotor, eine Diode, MOSFET Transistor, 9-Volt-Block Batterie und ein 100 $\mu$F Kondensator mit Potentiometer verwendet.\todo[author=anja,inline]{am besten nicht ganz so genau, aber mit projektbeschreibung vermischen (windrad wird über gleichstrommoter angetrieben, der über taster ein- und ausgeschaltet werden kann oder so)} In dem Einstiegsprojekt aus dem Buch\todo[author=anja]{projektnummer und quellenangabe (dann kann auch buch weg)} war es bisher nur möglich den Motor, mit einem Taster ein- und auszuschalten. Mit diesem Umbau kann der Nutzer durch Drehen am Potentiometer selbst entscheiden, mit welcher Geschwindigkeit der Motor und damit das Windrad drehen soll.
\\
